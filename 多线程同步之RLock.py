...
与Lock不同的是：在同一线程中，当调用了RLock的方法acquire()之后，可以在调用方法release()之前
...
如果计数器counter减1后变为0，则将锁的拥有者设置为None，然后释放锁。方法release()的定义如下：
def release(self):
    ...
    RLock相当于一个门可以上多把锁，上多少把锁就得开多少把锁。
    《图解Python》
    因此，方法acquire()和release()必须成对出现。如果在某个线程中调用了n次acquire()，
必须调用n次release()才能释放该线程所占用的锁。

    RLock也遵守了上下文管理协议，所以可以使用with语句。
"""
