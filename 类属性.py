...
"""
    ...
    2. 在类对象的外部
        ...
        类对象的所有实例对象都有一个指向类对象的指针，所以，类的所有实例对象都可以访问类属性。
...
访问属性时，会先查找指定的实例对象中有没有指定名称的实例属性，如果没有，再查找对应的类对象中有没有
指定名称的类属性。所以，当通过"实例对象.属性名"的方式访问属性时，如果实例属性和类属性同名，
实例属性会屏蔽掉类属性。
...
# 在实例对象p中没有找到名为age的实例属性，然后在类Person中找到了名为age的类属性
print(p.age)        # 18
# 打印名为age的类属性
print(Person.age)   # 18
...
# Person.age += 3等价于Person.age = Person.age + 3
