...
"""
    标准库模块threading中提供了一个类对象Semaphore，用于表示信号量，它可以帮助我们控制
并发线程的最大数量，从而实现多线程之间的同步。
...
放入一辆。
    在这个示例中，每个车位都是一个资源，每辆车都好比一个线程，看门人好比是信号量，入口处的等待区
好比是信号量等待池。在信号量的内部维护着一个计数器，其初始值是一个正整数（相当于车位数）。
当有线程（车）想要使用资源（车位）时，先调用方法acquire()试图获得资源（车位），如果此时计数器
大于0（有车位），则信号量（看门人）把计数器减1，然后为该线程（车）分配一个资源（车位）；
如果调用方法acquire()时计数器为0（没有车位），则对应的线程（车）被阻塞，然后被放到信号量等待池
（入口处的等待区）。当有其它线程（车）使用完资源（车位）时，调用方法release()释放资源（车位），
信号量（看门人）把计数器加1，如果此时计数器为1并且信号量等待池（入口处的等待区）有处于等待的线程（车），
则选择一个线程（车），信号量（看门人）把计数器减1，然后为该线程（车）分配一个资源（车位）。
    《图解Python》
...
（2）acquire(blocking=True, timeout=None)
    参数blocking用于指定获得资源失败时线程是否被阻塞，默认值True表示被阻塞，False表示不阻塞。
...
该方法返回True；如果计数器为0，线程被阻塞，然后被放到信号量等待池，直到有另外一个线程调用了
方法release()并且当前线程被线程调度程序从信号量等待池中选中，计数器减1然后返回True。
（3）release()    
